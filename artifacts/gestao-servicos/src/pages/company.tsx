...
import { AlertTriangle, Building2, Image, Loader2, Save, X } from 'lucide-react';
  ...
  return (
    <AppShell>
      <PageHeader title="Dados da empresa" description="Informações exibidas no cabeçalho dos laudos e ordens de serviço." actions={
        ...
        }))} disabled={isPending || isUploading} data-testid="button-save-company">
          {isPending || isUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Salvar
        </Button>
      } />
      <Form {...form}>
        <form onSubmit={form.handleSubmit((values) => updateCompany(values))} className="grid gap-5 lg:grid-cols-[1fr_280px]">
          <div className="rounded-xl border bg-card p-5 space-y-4">
            ...
            <FormField control={form.control} name="logoUrl" render={({ field }) => <FormItem>
                ...
                <Input
                  ...
                  onChange={async (event) => {
                    ...
                    try {
                      ...
                      toast({ title: 'Logo anexada', description: 'Clique em Salvar para confirmar a alteração.' });
                    } catch (error) {
                      toast({ title: 'Erro ao anexar logo', description: error instanceof Error ? error.message : 'Tente novamente.', variant: 'destructive' });
                    }
                  }}
                />
              </FormControl>
              <p className="text-xs text-muted-foreground">PNG, JPG ou outra imagem de até 10 MB.</p>
              {field.value && <Button type="button" variant="outline" size="sm" onClick={() => field.onChange('')}><X className="h-3.5 w-3.5" /> Remover logo</Button>}
              <FormMessage />
            </FormItem>} />
          </div>
          <div className="rounded-xl border bg-card p-5 h-fit">
            ...
            <div className="aspect-square rounded-lg border border-dashed bg-muted/30 flex items-center justify-center overflow-hidden">
              {logoUrl && !logoFailed ? <img src={getStoredImageUrl(logoUrl)} alt="Logo da empresa" className="max-h-full max-w-full object-contain p-4" onError={() => setLogoFailed(true)} /> : <div className="text-center text-muted-foreground"><Image className="h-10 w-10 mx-auto mb-2 opacity-40" /><p className="text-xs">{logoFailed ? 'Não foi possível carregar a imagem' : 'Anexe o logo da empresa'}</p></div>}
            </div>
