...
} from '@/components/material-form-dialog';
...
export default function MaterialsPage() {
  ...
  const handleSubmit = (values: MaterialFormValues) => {
    ...
    if (editingMaterial) {
      updateMaterial(editingMaterial.id, payload, () => setDialogOpen(false));
    } else {
      createMaterial(payload, () => setDialogOpen(false));
    }
  };
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={
          <Button onClick={handleOpenCreate} data-testid="button-new-material">
      ...
      {isLoading ? (
        ...
        <Empty className="border rounded-xl bg-card">
          ...
          <EmptyContent>
            <Button onClick={handleOpenCreate} data-testid="button-empty-new-material">
              <Plus className="h-4 w-4" />
              Cadastrar material
            </Button>
          </EmptyContent>
